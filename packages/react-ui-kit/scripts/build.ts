...
import configuration from "../builder.config.json";
...
const isFile = (path = "") => {
  ...
  return normalizedPath.includes(".") && normalizedPath.charAt(normalizedPath.length - 1) !== ".";
};
...
for (const source of Object.keys(configuration.files)) {
  ...
  const { fileName: destinationDileName, directory } = pathToWriteOptions(destination);

  fs.mkdirSync(directory, { recursive: true });

  fs.cpSync(source, destinationDileName ? `${directory}/${destinationDileName}` : `${directory}/${sourceFileName}`);
}
...
if (configuration.tailwind) {
  ...
  spawnSync("npx", [`tailwindcss`, "-o", `${distDir}/styles.min.css`, "build", "--minify"]);
}
