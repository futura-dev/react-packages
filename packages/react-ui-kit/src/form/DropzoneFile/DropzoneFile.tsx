...
export const DropzoneFile = ({ ...props }: DropzoneFileProps) => {
  const { id, label, file, name, reset, errorMessage, invalid = false, title = "", subTitle = "" } = props;
        ...
        >
          {file ? (
            <div className="relative h-full w-full flex justify-center">
              <a id={id} className="flex flex-col items-center self-center" href={urlFile}>
                <IconFile width="50" height="50" viewBox="0 0 24 24" color="#9CA3AF" />
                <span className="mt-3 dark:text-white">{file.name}</span>
              </a>
              <button className="absolute end-5 top-4" onClick={() => reset && reset(name)}>
                <IconTrash width="30" height="30" viewBox="0 0 24 24" color="#6C09EF" />
              </button>
            </div>
          ) : (
            <>
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <IconCloudUpload width="50" height="50" viewBox="0 0 24 24" color="#9CA3AF" />
                <p className="mb-2 futura-text-extra-small text-gray-500 dark:text-gray-400 mt-2">
                  <span className={classNames(invalid ? "text-red-500" : "text-futura-purple dark:text-white", "font-semibold")}>{title}</span>
                </p>
